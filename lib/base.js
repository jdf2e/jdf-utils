...
var $ = module.exports = {
    reg: {
        widget: function() {
            //检测是否存在和取widget name
            return new RegExp('{%widget\\s*.*?\\sname="(.*?)".*?%}', 'gm');
        },
        widgetType: function() {
            //取widget type
            return new RegExp('{%widget\\s*.*?\\stype="(.*?)".*?%}', 'gm');
        },
        widgetData: function() {
            //取widget data
	    return new RegExp('{%widget\\s*.*?\\sdata=\'(.*|(?:.*?\\\'{1}.*?)*)\'.*?%}', 'gm');
//             return new RegExp('{%widget\\s*.*?\\sdata=\'(.*?)\'.*?%}', 'gm');
        },
        widgetComment: function() {
            //取widget 是否有注释
            return new RegExp('{%widget\\s*.*?\\scomment=[\'|"](.*?)[\'|"].*?%}', 'gm');
        },
        widgetPosition: function() {
            //取widget position
            return new RegExp('{%widget\\s*.*?\\sposition="(.*?)".*?%}', 'gm');
        },
